...
export const roomInMove = async (db: AppDb, moveId: string, id: string): Promise<boolean> =>
  Boolean((await db.select({ i: s.rooms.id }).from(s.rooms).where(and(eq(s.rooms.id, id), eq(s.rooms.moveId, moveId))).limit(1))[0]);

export const boxInMove = async (db: AppDb, moveId: string, id: string): Promise<boolean> =>
  Boolean((await db.select({ i: s.boxes.id }).from(s.boxes).where(and(eq(s.boxes.id, id), eq(s.boxes.moveId, moveId))).limit(1))[0]);

export const statusInMove = async (db: AppDb, moveId: string, id: string): Promise<boolean> =>
  Boolean((await db.select({ i: s.statuses.id }).from(s.statuses).where(and(eq(s.statuses.id, id), eq(s.statuses.moveId, moveId))).limit(1))[0]);

export const markerInMove = async (db: AppDb, moveId: string, id: string): Promise<boolean> =>
  Boolean((await db.select({ i: s.markers.id }).from(s.markers).where(and(eq(s.markers.id, id), eq(s.markers.moveId, moveId))).limit(1))[0]);
