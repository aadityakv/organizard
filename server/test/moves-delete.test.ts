...
describe('DELETE /v1/moves/:id', () => {
  ...
  it('owner delete removes all child rows (cascade)', async () => {
    ...
    const muts: Mutation[] = [
      ...
      mut('addItem', { id: 'item1', boxId: 'box1', name: 'Skillet', qty: 1, valueCents: 8000, markerIds: ['mk1'] }, 'c5'),
    ];
    ...
    const byMove = [schema.boxes, schema.items, schema.rooms, schema.markers, schema.statuses, schema.members] as const;
    ...
    expect(await h.db.select().from(schema.itemMarkers).where(eq(schema.itemMarkers.itemId, 'item1'))).toEqual([]);
  });
});
