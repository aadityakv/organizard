...
type Snap = { move: { id: string }; statuses: { id: string }[]; markers: { id: string }[]; boxes: { id: string; markerIds: string[] }[]; items: { id: string }[] };
...
describe('security — cross-move isolation (IDOR)', () => {
  it('mutations on move A cannot touch move B rows even for a member of both', async () => {
    ...
    await h.json(`/v1/moves/${b.move.id}/mutations`, {
      mutations: [m('addBox', { id: 'bB', roomId: 'rB', number: 1, name: 'B box', color: 'sky', statusId: b.statuses[0].id }, 'x0')],
    }, auth(owner.session));
    // addBox needs its room to exist first -> create the room, then the box
    await h.json(`/v1/moves/${b.move.id}/mutations`, { mutations: [m('addRoom', { id: 'rB', name: 'RB', icon: 'box' }, 'x1')] }, auth(owner.session));
    await h.json(`/v1/moves/${b.move.id}/mutations`, {
      mutations: [m('addBox', { id: 'bB', roomId: 'rB', number: 1, name: 'B box', color: 'sky', statusId: b.statuses[0].id }, 'x2')],
    }, auth(owner.session));
  ...
  it('addBox referencing a foreign room/status is skipped', async () => {
    ...
    await h.json(`/v1/moves/${a.move.id}/mutations`, {
      mutations: [m('addBox', { id: 'bA', roomId: 'rB', number: 1, name: 'x', color: 'green', statusId: b.statuses[0].id }, 'a1')],
    }, auth(owner.session));
