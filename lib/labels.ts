// ============================================================
// Printable QR labels. Builds a print-ready HTML sheet of box
// labels (QR + box #, name, room) and hands it to expo-print,
// which opens the native iOS print/share dialog.
//
...
/**
 * Build the label sheet and open the native print dialog. Wrapped so a user
 * cancelling the dialog (which rejects on iOS) doesn't surface as an error.
 * Returns true if printing was dispatched, false if it was cancelled/failed.
 */
export async function printLabels(labels: LabelInput[]): Promise<boolean> {
  if (labels.length === 0) return false;
  try {
    await Print.printAsync({ html: buildLabelsHtml(labels) });
    return true;
  } catch {
    // User cancelled the print sheet, or no printer flow available — stay calm.
    return false;
  }
}
