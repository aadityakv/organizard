...
export default function StreamSession() {
  ...
  const sessionValue = useMemo(() => session.reduce((a, it) => a + (it.value || 0) * (it.qty || 1), 0), [session]);
  ...
  const finalizeList = (raw: string) => {
    ...
    const batch: SItem[] = parsed.map((p) => ({
      id: uid('v'), name: p.name, qty: p.qty, value: p.value, icon: iconFor(p.name), needsFix: false, boxId,
    }));
  ...
  const finalize = (raw: string) => {
    ...
    if (target) {
      setSession((prev) =>
        prev.map((it) =>
          it.id === target
            ? { ...it, name: p.name || it.name, qty: p.qty ?? it.qty, value: p.value ?? it.value, icon: iconFor(p.name || it.name), needsFix: !p.name && it.needsFix }
            : it,
        ),
      );
      setLastId(target);
  ...
  const finish = () => {
    ...
    router.replace({ pathname: `/box/${initialBoxId ?? boxId}`, params: { streamed: String(session.length) } });
  };
    ...
    <View style={styles.root}>
        ...
        <View style={styles.pillRow}>
          {mic === 'listening' ? (
            <View style={styles.pillListening}>
              ...
              <Text style={styles.pillListenText} numberOfLines={1}>{'“' + (transcript || '…') + '”'}</Text>
            </View>
          ) : null}
          {mic === 'gotit' && (lastBatch > 1 || (lastIt && !lastIt.needsFix)) ? (
            <View style={styles.pillGot}>
              <Icon name="check" size={16} color={palette.green700} />
              <Text style={styles.pillGotText}>{lastBatch > 1 ? `Added ${lastBatch} items` : `Got it — ${lastIt?.name}`}</Text>
            </View>
          ) : null}
          {mic === 'fail' ? (
        ...
        {lastIt && !flash ? (
          <Pressable onPress={() => setEditId(lastIt.id)} style={styles.lastCard}>
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text numberOfLines={1} style={[styles.lastName, lastIt.needsFix && { color: palette.amber600 }]}>{lastIt.name}</Text>
            ...
            <View style={{ alignItems: 'flex-end', gap: 4 }}>
              <View style={styles.addedRow}><Icon name="check" size={14} color={palette.green700} /><Text style={styles.addedText}>Added</Text></View>
        ...
        <View style={{ gap: 6 }}>
          {boxes.map((b) => (
            <Pressable key={b.id} onPress={() => { setBoxId(b.id); setPickerOpen(false); }} style={[styles.pickRow, b.id === boxId && { backgroundColor: palette.green50 }]}>
              ...
              <Text style={styles.pickLabel}>#{b.number} {b.name}</Text>
      ...
      <Sheet visible={ledgerOpen} onClose={() => setLedgerOpen(false)} title="This session">
        <Text style={styles.ledgerTotals}>
          {session.length} {session.length === 1 ? 'item' : 'items'} · {money(sessionValue)}{fixCount ? ` · ${fixCount} to fix` : ''}
        </Text>
        <ScrollView style={{ maxHeight: 320 }}>
          ...
          {[...session].reverse().map((it) => (
            <Pressable key={it.id} onPress={() => setEditId(it.id)} style={[styles.ledgerRow, it.needsFix && { backgroundColor: palette.amber50 }]}>
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text numberOfLines={1} style={[styles.ledgerName, it.needsFix && { color: palette.amber600 }]}>{it.name}</Text>
                {it.needsFix ? <Text style={styles.ledgerFix}>That doesn't look right — tap to fix</Text> : null}
              </View>
        ...
        <Button fullWidth onPress={() => setLedgerOpen(false)} style={{ marginTop: 12 }}>Back to camera</Button>
      </Sheet>
            ...
            <View style={{ flexDirection: 'row', gap: 14 }}>
              <View style={{ flex: 1, gap: 6 }}>
                ...
                <View style={styles.valInputWrap}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(v) => { const c = v.replace(/[^0-9.]/g, ''); patchEdit({ value: c === '' ? null : parseFloat(c) }); }}
              ...
              <View style={{ gap: 6 }}>
                ...
                <View style={styles.stepper}>
                  <Pressable onPress={() => patchEdit({ qty: Math.max(1, (editIt.qty || 1) - 1) })} style={styles.stepBtn}><Text style={styles.stepTxt}>−</Text></Pressable>
                  <Text style={styles.qtyVal}>{editIt.qty || 1}</Text>
                  <Pressable onPress={() => patchEdit({ qty: (editIt.qty || 1) + 1 })} style={styles.stepBtn}><Text style={styles.stepTxt}>+</Text></Pressable>
                </View>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  voiceRing: { width: 150, height: 150, borderRadius: 75, borderWidth: 3, borderColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  voiceRingInner: { position: 'absolute', top: 18, left: 18, right: 18, bottom: 18, borderRadius: 75, borderWidth: 2, borderColor: 'rgba(255,255,255,0.12)' },
  hint: { color: 'rgba(255,255,255,0.7)', fontFamily: fonts.body.bold, fontSize: 13, textAlign: 'center' },
  simNote: { color: 'rgba(255,255,255,0.45)', fontFamily: fonts.body.bold, fontSize: 11.5, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 999, paddingVertical: 5, paddingHorizontal: 12, overflow: 'hidden' },
  pillRow: { alignItems: 'center', marginTop: 26, minHeight: 44, justifyContent: 'center', paddingHorizontal: 16 },
  pillListening: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: colors.brand, borderRadius: 999, paddingVertical: 10, paddingHorizontal: 18, maxWidth: '100%' },
  ...
  pillGot: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: palette.green50, borderRadius: 999, paddingVertical: 10, paddingHorizontal: 18 },
  pillGotText: { color: palette.green700, fontFamily: fonts.body.extra, fontSize: 14 },
  pillFail: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: palette.amber50, borderRadius: 999, paddingVertical: 10, paddingHorizontal: 18 },
  pillFailText: { color: palette.amber600, fontFamily: fonts.body.extra, fontSize: 13 },
  lastCard: { position: 'absolute', left: 16, right: 16, bottom: 150, backgroundColor: colors.surfaceCard, borderRadius: 18, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 12, shadowColor: '#000', shadowOpacity: 0.35, shadowRadius: 18, shadowOffset: { width: 0, height: 10 }, elevation: 12 },
  ...
  qtyChip: { backgroundColor: palette.cream200, color: palette.ink700, borderRadius: 999, paddingVertical: 2, paddingHorizontal: 9, fontSize: 11.5, fontFamily: fonts.body.extra, overflow: 'hidden' },
  valChip: { backgroundColor: palette.amber50, color: palette.amber600, borderRadius: 999, paddingVertical: 2, paddingHorizontal: 9, fontSize: 11.5, fontFamily: fonts.body.extra, overflow: 'hidden' },
  ...
  undo: { color: palette.ink400, fontFamily: fonts.body.extra, fontSize: 12, textDecorationLine: 'underline' },
  bottom: { position: 'absolute', left: 0, right: 0, bottom: 30, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 28 },
  bottomSide: { flex: 1 },
  strip: { flexDirection: 'row', alignItems: 'center', minHeight: 44 },
  thumb: { width: 44, height: 44, borderRadius: 12, borderWidth: 2, borderColor: 'rgba(255,255,255,0.9)', alignItems: 'center', justifyContent: 'center' },
  countPill: { backgroundColor: 'rgba(255,255,255,0.16)', color: '#fff', borderRadius: 999, paddingVertical: 4, paddingHorizontal: 10, fontSize: 12, fontFamily: fonts.body.extra, overflow: 'hidden' },
  ...
  redo: { width: 52, height: 52, borderRadius: 26, backgroundColor: 'rgba(255,255,255,0.16)', alignItems: 'center', justifyContent: 'center' },
  ...
  empty: { paddingVertical: 24, textAlign: 'center', color: palette.ink400, fontFamily: fonts.body.bold, fontSize: 13.5 },
  ledgerRow: { flexDirection: 'row', alignItems: 'center', gap: 11, padding: 8, borderRadius: 12, marginBottom: 2 },
  ...
  input: { height: 48, borderWidth: 1.5, borderColor: palette.sand300, borderRadius: 14, paddingHorizontal: 14, fontFamily: fonts.body.bold, fontSize: 16, color: palette.ink900 },
  valInputWrap: { flexDirection: 'row', alignItems: 'center', height: 48, borderWidth: 1.5, borderColor: palette.sand300, borderRadius: 14, paddingHorizontal: 14, gap: 8 },
  ...
  stepper: { flexDirection: 'row', alignItems: 'center', gap: 4, height: 48, borderWidth: 1.5, borderColor: palette.sand300, borderRadius: 14, paddingHorizontal: 6 },
  stepBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: palette.cream200, alignItems: 'center', justifyContent: 'center' },
  stepTxt: { fontSize: 18, fontFamily: fonts.body.extra, color: palette.ink700 },
  qtyVal: { minWidth: 32, textAlign: 'center', fontFamily: fonts.body.extra, fontSize: 16, color: palette.ink900 },
  summaryScrim: { position: 'absolute', inset: 0, backgroundColor: 'rgba(42,39,34,0.55)', alignItems: 'center', justifyContent: 'center', padding: 28 },
  summaryCard: { backgroundColor: colors.surfaceCard, borderRadius: 24, padding: 28, width: '100%', alignItems: 'center', gap: 10 },
  summaryTitle: { fontFamily: fonts.display.bold, fontSize: 22, color: palette.ink900 },
  summaryLabel: { fontSize: 14, fontFamily: fonts.body.bold, color: palette.ink500, textAlign: 'center', lineHeight: 20 },
});
